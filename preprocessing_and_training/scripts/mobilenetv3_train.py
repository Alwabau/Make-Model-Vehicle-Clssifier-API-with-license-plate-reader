"""
This script will be used for training our ModileNet CNN. This model will 
classify between exterior and interior vahicules images. The only input 
argument it should receive is the path to our configuration file in which 
we define all the experiment settings like dataset, model output folder, 
epochs, learning rate, data augmentation, etc.
"""
...
def main(config_file):
    ...
    # Creates a MobileNetv3 model for finetuning
    cnn_model = mobilenetv3_model.create_model(**config["model"])
    ...
    cnn_model.fit(
        train_ds, validation_data=val_ds, callbacks=callbacks, **config["fit"], verbose = 2
    )
